...
execute if score num clear matches 1 at @e[limit=1,tag=ramdon] as @r run item modify block ~ 131 ~ container.0 {"function":"minecraft:set_name","entity":"this","name":[{"selector":"@a[scores={clear=0}]","color":"yellow","bold":true}," ",{"text":"遭到處決！","color":"yellow","bold":true}]}
execute if score num clear matches 1 at @e[limit=1,tag=ramdon] run data remove block ~ 131 ~ Items.[{Slot:0b}].components.minecraft:custom_name.click_event
...
execute if score num clear matches 2.. run data modify storage rlk:spy_vote_log records append value {"text":"無人被處決","color":"yellow","bold":true}
