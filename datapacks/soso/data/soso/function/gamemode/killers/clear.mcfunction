...
execute unless score clear zTEST matches 0.. store result bossbar minecraft:clear max store result bossbar minecraft:clear value run scoreboard players set clear zTEST 1600
...
execute if score 遊戲開始 zTEST matches 1 if score clear zTEST matches -1 run item replace entity @a[team=Killer] container.35 with minecraft:wither_rose[unbreakable={},custom_name=[{"text":""},{"text":"暗夜時間","color":"gold","bold":true}],lore=["進入夜晚","全體隱身八十秒、十秒後發光60秒。"],custom_data={skill:1b,night:1b}]
execute if score 遊戲開始 zTEST matches 1 if score clear zTEST matches -1 run tellraw @a[team=Killer] {"text":"暗夜時刻回歸你的包包裡了，再次進行殺戮吧","color":"blue","bold":true}
