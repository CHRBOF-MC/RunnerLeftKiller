fill ~ ~ ~ ~ ~ ~ minecraft:air
tag @e remove killit
execute as @e[tag=spawnkiller,limit=1,sort= nearest] at @s run tag @s add killit
...
tellraw @p ["",{"text":"確定要刪除該物件請點【刪除該物件】","bold":true,"color":"dark_blue","click_event":{"action":"run_command","command":"/function soso:game/spawn/buttonkillb"}}]
