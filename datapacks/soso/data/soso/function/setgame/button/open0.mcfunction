scoreboard players remove @e[limit=1,sort=nearest,tag=button,distance=..10] deathscore 5
title @s actionbar ["",{"text":"逃生門開啟時間設定為：","bold":false,"color":"green"},{"score":{"name":"@e[limit=1,sort=nearest,tag=button,distance=..10]","objective":"deathscore"}},{"text":"秒","bold":false,"color":"green"}]
