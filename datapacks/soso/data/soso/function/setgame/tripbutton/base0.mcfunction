scoreboard players remove @e[limit=1,sort=nearest,tag=buttonbreak,distance=..10] CODENAMEb 1
title @s actionbar ["",{"text":"陷阱按鈕編號設定為：","bold":false,"color":"green"},{"score":{"name":"@e[limit=1,sort=nearest,tag=buttonbreak,distance=..10]","objective":"CODENAMEb"}},{"text":"號","bold":false,"color":"green"}]
