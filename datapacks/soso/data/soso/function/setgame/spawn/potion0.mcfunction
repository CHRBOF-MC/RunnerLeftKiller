scoreboard players remove @e[limit=1,sort=nearest,tag=spawn,distance=..10] potion 1
title @s actionbar ["",{"text":"藥水推薦數設定為：","bold":true,"color":"light_purple"},{"score":{"name":"@e[limit=1,sort=nearest,tag=spawn,distance=..10]","objective":"potion"}}]
